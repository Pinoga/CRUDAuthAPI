...
const UserController = {
	...
	signIn: (req: Request, res: Response, next: NextFunction) => {
		res.cookie('token', signToken(req.user as User), { httpOnly: true, sameSite: false });
		res.locals.statusCode = 200;
	...
	signUp: async (req: Request, res: Response, next: NextFunction) => {
		...
		try {
			...
			res.cookie('token', signToken(user), { httpOnly: true, sameSite: false });
			res.locals.statusCode = 200;
			res.locals.data = user;
			next();
		} catch (error) {
	...
	// Se usuário está autenticado, retorna os dados do usuário
